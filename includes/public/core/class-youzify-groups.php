...
class Youzify_Group {
	...
	function statistics( $args = null ) {
		...
		<div class="<?php echo esc_attr( youzify_generate_class( $statistics_class ) ); ?>">
			<ul>
				...
				<?php if ( 'on' == $display_posts && bp_is_active( 'activity' ) ) : ?>
					<li>
						<div class="youzify-snumber" title="<?php echo esc_attr( $posts_number ); ?>"><?php echo $this->get_statistic_number( $posts_number ); ?></div>
				...
				<?php if ( 'on' == $display_members ) : ?>
					...
					<li>
						<div class="youzify-snumber" title="<?php echo esc_attr( $members_number ); ?>"><?php echo $this->get_statistic_number( $members_number ); ?></div>
	...
	function get_group_membership_requests_count( $group_id ) {
		...
		// Result.
		$requests_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$bp->groups->table_name_members} WHERE is_confirmed = 0 AND inviter_id = 0 AND group_id = %d ", $group_id ) );
