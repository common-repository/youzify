...
class Youzify_Attachments {
	...
	function get_privacy( $activity_id ) {
		...
		$privacy = $wpdb->get_var( $wpdb->prepare( "SELECT privacy from {$bp->activity->table_name} WHERE id = %d", $activity_id ) );
	...
	function save_media_attachments( $item_id, $attachments, $component ) {
		...
		switch ( $component ) {
			...
			case 'comment':
				global $bp;
				$comment_activity_id = $wpdb->get_var( $wpdb->prepare( "SELECT item_id from {$bp->activity->table_name} WHERE id = %d", $item_id ) );
	...
	function get_compressed_image( $source, $filename ) {
	    ...
	    $compressed_image_filename = apply_filters( 'youzify_attachments_compression_image_filename', $file_name );
	    
	    // Get New Image Path.
	    $compressed_image = wp_unique_filename( $folder['path'], $compressed_image_filename . '.jpg' );
