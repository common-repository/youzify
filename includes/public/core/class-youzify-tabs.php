...
class Youzify_Tabs {
	...
	function add_profile_tabs() {
		...
		if ( apply_filters( 'youzify_add_profile_courses_tab', false ) ) {
			...
			if ( bp_is_my_profile() || youzify_option( 'youzify_make_courses_tab_public', 'off' ) == 'on' || is_super_admin() ) {
			    ...
			    bp_core_new_subnav_item(
			    	array(
			            'slug' => 'all',
			            'parent_slug' => 'courses',
			            'name' => __( 'Enrolled Courses', 'youzify' ),
			...
			if ( apply_filters( 'youzify_add_profile_certificates_subtab', false ) )  {
    ...
    function courses_screen() {
        ...
        add_action( 'bp_template_content', array( $this, 'courses_tab' ) );
    ...
    function certificates_screen() {
        ...
        add_action( 'bp_template_content', array( $this, 'certificates_tab' ) );
    ...
    function courses_tab() {
    	do_action( 'youzify_profile_courses_tab_content' );
    }
    ...
    function certificates_tab() {
    	do_action( 'youzify_profile_certificates_tab_content' );
    }
