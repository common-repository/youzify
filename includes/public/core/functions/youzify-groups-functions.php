...
function youzify_get_group_total_posts_count( $group_id ) {
	...
	$total_updates = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$bp->activity->table_name} WHERE component = 'groups' AND item_id = %d AND type NOT IN ( 'created_group', 'joined_group' ) ", $group_id ) );
