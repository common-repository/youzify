...
add_action( 'bp_actions', 'youzify_init_woocommerce' );
...
/**
 * Clear Woocomerce Total User Products Count
 * */
function youzify_get_user_woocommerce_products_count( $user_id ) {
    $cache_key = 'youzify_woocommerce_user_products_count_' . $user_id;
    $cached_products_count = get_transient( $cache_key );

    if ( $cached_products_count === false ) {
        global $wpdb;
        $products_count = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}posts
            WHERE post_type = 'product' AND post_author = %d AND post_status = 'publish'",
            $user_id
        ) );
        set_transient( $cache_key, $products_count, 12 * HOUR_IN_SECONDS ); // Cache for 12 hours
    } else {
        $products_count = $cached_products_count;
    }
...
/**
 * Clear Woocomerce Total User Products Cache
 * */
function youzify_clear_user_woocommerce_products_cache( $post_id ) {
...
add_action( 'save_post_product', 'youzify_clear_user_woocommerce_products_cache' );

/**
 * Get User WooCommerce Statistics
 * */
add_filter( 'youzify_get_user_statistic_number', 'youzify_wc_add_products_statistics', 10, 3 );

function youzify_wc_add_products_statistics( $value, $user_id, $type ) {

    switch ( $type ) {

        case 'products':
            $value = youzify_get_user_woocommerce_products_count( $user_id );
            break;
...
/**
 * Get User WooCommerce Statistics Posts and Comments Type
 * */
add_filter( 'youzify_get_user_statistics_options', 'youzify_wc_add_user_statistics_options' );

function youzify_wc_add_user_statistics_options( $statistics ) {
    $statistics['products'] = __( 'Products', 'youzify' );
    return $statistics;
}

/**
 * Get User WooCommerce Statistics Products Type
 * */
add_filter( 'youzify_get_user_statistics_details', 'youzify_wc_add_user_statistics_details', 10, 2 );

function youzify_wc_add_user_statistics_details( $statistics, $user_id ) {

    $profile =  bp_members_get_user_url( $user_id );

    // Get Comments Args
    $statistics['products'] = array(
        'title' => __( 'Products', 'youzify' ),
        'link'  => $profile,
    );
