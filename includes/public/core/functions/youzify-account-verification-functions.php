...
function youzify_get_verified_users( $args = null ) {
	...
	// Request.
	$request = $wpdb->prepare( "SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = 'youzify_account_verified' AND meta_value = '%s'", $verified );

	$order_by = isset( $args['order_by'] ) && $args['order_by'] == 'random' ? 'RAND()' : $args['order_by'];

	if ( isset( $args['order_by'] ) ) {
		$request .= $wpdb->prepare( " ORDER BY %1s", $order_by );
	}

	if ( isset( $args['limit'] ) ) {
		$request .= $wpdb->prepare( " LIMIT %d", $args['limit'] );
	}
