...
/**
 * Get User BBpress Statistics
 * */
add_filter( 'youzify_get_user_statistic_number', 'youzify_add_forums_statistics', 10, 3 );

function youzify_add_forums_statistics( $value, $user_id, $type ) {

    switch ( $type ) {

        case 'forum_posts':        
            $value = bbp_get_user_topic_count_raw( $user_id );
            break;

        case 'forum_comments':
            $value = bbp_get_user_reply_count_raw( $user_id );
            break;
...
/**
 * Get User BBpress Statistics Posts and Comments Type
 * */
add_filter( 'youzify_get_user_statistics_options', 'youzify_bbpress_add_user_statistics_options' );

function youzify_bbpress_add_user_statistics_options( $statistics ) {
    $statistics['forum_posts'] = __( 'Forum Topics', 'youzify' );
    $statistics['forum_comments'] = __( 'Forum Replies', 'youzify' );
    return $statistics;
}
 ...
 * Get User BBpress Statistics Posts and Comments Type
 * */
add_filter( 'youzify_get_user_statistics_details', 'youzify_bbpress_add_user_statistics_details', 10, 2 );

function youzify_bbpress_add_user_statistics_details( $statistics, $user_id ) {

    $profile =  bp_members_get_user_url( $user_id );

    // Get Comments Args
    $statistics['forum_comments'] = array(
        'title' => __( 'Replies', 'youzify' ),
        'link'  => $profile . 'forum',
    );

    // Get Posts Args
    $statistics['forum_posts'] = array(
        'title' => __( 'Topics', 'youzify' ),
        'link'  => $profile . 'forum',
    );
