...
function youzify_get_users_list( $users, $args = null ) {
    ...
    <div class="youzify-items-list-widget youzify-list-avatar-circle <?php echo youzify_generate_class( $main_class ); ?>">
        ...
        <div class="youzify-list-item">
            <a href="<?php echo esc_url( $profile_url ); ?>" class="youzify-item-avatar"><?php echo bp_core_fetch_avatar( array( 'item_id' => $user_id, 'type' => 'thumb' ) ); ?></a>
            <div class="youzify-item-data">
                <a href="<?php echo esc_url( $profile_url ); ?>" class="youzify-item-name"><?php echo bp_core_get_user_displayname( $user_id ); ?><?php youzify_the_user_verification_icon( $user_id ); ?></a>
...
function youzify_convert_content_tags( $content ) {
    ...
    // Get User Displayed Username
    $displayed_username = bp_members_get_user_slug( $displayed_user_id );

    if ( ! empty( $displayed_username ) ) {
        $content = str_replace( '{displayed_username}', $displayed_username, $content );
    }

    // Get User Displayed User ID
    if (  ! empty( $displayed_user_id ) ) {
        $content = str_replace( '{displayed_user_id}', $displayed_user_id, $content );
    }

    $logged_in_user = bp_members_get_user_slug( bp_loggedin_user_id() );

    if ( ! empty( $logged_in_user ) ) {
        $content = str_replace( '{logged_in_user}', $logged_in_user, $content );
    }
 ...
 * Tab Content
 */
function youzify_get_user_certificates( $certificates = '' ) {

    // Load Profile Settings CSS.
    // wp_enqueue_style( 'youzify-courses-certificates', YOUZIFY_ASSETS . 'css/youzify-certificates.min.css', false, YOUZIFY_VERSION );

    if ( ! empty ( $certificates ) ) :

        echo '<div class="youzify-user-certificates" style="display:none;">';

        foreach ( $certificates as $certificate ) : ?>

            <div class="youzify-user-certificate">
                <div class="youzify-user-certificate-content">
                    <?php if ( ! empty( $certificate['course_image'] ) ) : ?>
                    <div class="youzify-cetificate-image">
                        <img class="youzify-cetificate-main-image" loading="lazy" src="<?php echo $certificate['course_image']; ?>" alt=''>
                        <img class="youzify-cetificate-badge-image" src="<?php echo YOUZIFY_ASSETS;?>images/certificate.png" alt="">
                    </div>
                    <?php else : ?>
                        <img src="<?php echo YOUZIFY_ASSETS;?>images/certificate.png" alt="">
                    <?php endif; ?>
                    <div class="youzify-certificate-data">
                        <div class="youzify-certificate-data-item">
                            <div class="youzify-certificate-label"><?php _e( 'Certificate in', 'youzify' ); ?></div>
                            <a class="youzify-certificate-value" href="<?php echo esc_url( $certificate['course_url'] ); ?>"><?php echo $certificate['course_title']; ?></a>
                        </div>
                        <div class="youzify-certificate-data-item">
                            <div class="youzify-certificate-label"><?php _e( 'Issued on', 'youzify' ); ?></div>
                            <div class="youzify-certificate-value"><?php echo $certificate['issued_on']; ?></div>
                        </div>
                    </div>
                </div>
                <a class="youzify-certificate-action" target="_blank" href="<?php echo esc_url( $certificate['url'] ); ?>"><i class="fas fa-award"></i><?php _e( 'View Certificate', 'youzify' ); ?></a>
            </div>
        ...
        echo '<div class="youzify-info-msg youzify-failure-msg"><div class="youzify-msg-icon"><i class="fas fa-exclamation-triangle"></i></div>
            <p>'. __( 'Sorry, no certificates found!', 'youzify' ) . '</p></div>';
    endif;
