...
class Youzify_Fields {
	...
	function get_field( $option, $is_user = false, $options_name = 'youzify_options' ) {
		...
		if ( 'open' != $option['type']  && 'close' != $option['type']  && ! empty( $option['id'] ) ) {
			...
			if ( ! $is_user ) {
				// Get Option Value.
				if ( $options_name == 'youzify_options' ) {
					$option_value = $this->get_option( $option['id'] );
				} else {
					$option_value = $this->get_option( $options_name );
					$option_value = ! empty( $option_value ) && isset( $option_value[ $option['id'] ] ) ? $option_value[ $option['id'] ] : '';
				}
				$user_defined_value = ! empty( $option_value ) ? $option_value : $default_value;
			} else {
				// Get Value From User Meta.
				// if ( $options_name == 'youzify_options' ) {
					// User for User notifications!
					// $data_value = get_the_author_meta( $options_name, bp_displayed_user_id(), true );
					// $data_value = ! empty( $data_value ) && isset( $data_value[ $option['id'] ] ) ? $data_value[ $option['id'] ] : '';
				// } else {
		...
		case 'connect':
			...
			<div class="<?php echo $connect_class; ?>" >
				...
				<div class="youzify-user-provider-box">
					...
					<div class="youzify-user-provider-unlink" data-youzify-tooltip="<?php _e( 'Unlink Account', 'youzify' ); ?>" data-provider="<?php echo $option['provider']; ?>" data-nonce="<?php echo $ajax_nonce; ?>" data-user_id="<?php echo bp_displayed_user_id(); ?>"><i class="fas fa-trash-alt"></i></div>
				</div>
				<?php endif; ?>

				<div class="youzify-connect-btn youzify-connect-btn-<?php echo $provider;?>">
					<a href="<?php echo esc_url( home_url( '/youzify-auth/feed/' . $option['provider'] ) );?>"><i class="<?php echo $option['icon']; ?>"></i><?php echo $option['button']; ?></a>
				</div>
