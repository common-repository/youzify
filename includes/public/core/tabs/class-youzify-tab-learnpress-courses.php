...
class Youzify_LearnPress_Courses_Tab {
...
// Count total user completed course of author
		...
		$total_rows     = 0;
		$filter         = apply_filters( 'lp/api/profile/courses/purchased/filter', $filter, array() );
	...
	function courses_core( $args, $user_id, $activity_type = '' ) {
		...
		foreach ( $blogs_ids as $b ) {
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$course_data = $user->get_course_data( $post_id );
				
				if ( ! $course_data ) {
					continue;
				}
				
				$progress = $course_data->get_result();
			...
			<div class="youzify-tab-course youzify-course-content">
				...
				<?php
					...
					} elseif ( $progress['result'] > 0 && $progress['result'] < 100 ) {
						$progress_status = __( 'In Progress', 'youzify' );
						$progress_id = 'in_progress';
					} elseif ( $progress['result']  == 100 ) {
				...
				<div class="youzify-course-container">
					...
					<div class="youzify-course-inner-content">
						...
						<div class="youzify-course-head">
							<?php if ( $activity_type != 'new_learnpress_course' && $display_enrolment_status == 'on' ) : ?>
							...
							<div class="youzify-course-meta">
								...
								<ul>
									<?php if ( $activity_type != 'new_learnpress_course' && 'on' == $display_author ) : ?>
									<li>
						...
						<?php endif; ?>
							<?php if (  $activity_type != 'new_learnpress_course' ) : ?>
							<div class="youzify-course-completion-data">

								<?php if ( $display_completion_bar == 'on' ) : ?>
									<div class="youzify-completionbar clearfix" data-percent="<?php echo $progress['result']; ?>" loaded="true">
										<div class="youzify-completion-bar" style="background-color: rgb(129, 215, 66); width: <?php echo $progress['result']; ?>%;">
										</div>
									</div>
								<?php endif; ?>
								...
								<div class="youzify-course-completion-meta">
									...
									<div class="youzify-course-bar-percent"><?php echo sprintf( __( '<span class="youzify-course-meta-label">Completed:</span> <span class="youzify-course-meta-value">%d%%</span>', 'youzify' ), $progress['result'] ) ?></div>
									<?php endif; ?>

									<?php if ( $display_completion_steps == 'on' ) : ?>
									<span class="youzify-course-progress-steps"><?php echo sprintf( __( '<span class="youzify-course-meta-label">Lessons:</span> <span class="youzify-course-meta-value">%s</span>', 'youzify' ), $progress['completed_items']  . '/' . $progress['count_items'] ); ?>
									</span>
									<?php endif; ?>
